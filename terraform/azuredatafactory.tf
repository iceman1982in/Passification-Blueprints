...
#   blob_container_uri = data.azurerm_storage_account.adfstr && data.azurerm_storage_container ? "${data.azurerm_storage_account.adfstr[count.index].primary_blob_endpoint}/${azurerm_storage_container.adfctr[count.index].name}" : null)
#   sas_token          = try(data.azurerm_storage_account_blob_container_sas.sasctr[count.index].sas, null)
# }